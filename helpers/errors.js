...
const errores = (error) => {
    ...
    if (error.isJoi) {
        ...
        const joiErr = {
            ...
            'object.base': {
                message: `'${contextKey}' debe ser un objeto`,
                boomStatus: 'notAcceptable'
            },
            'object.schema': {
            ...
            'object.missing': {
                message: `'${contextKey}' debe contener ${Array.isArray(contextPeers)
                    ? contextPeers.map((peer, index, arr) => index !== arr.length || index !== arr.length + 1
                        ? `${peer},`
                        : (index !== arr.length ? peer : `o ${peer}`)).join(' ')
                    : contextPeers}`
            },
        }
        ...
        throw customErr
            ...
            : Boom.badRequest(details.map(({ message }) => message.trim()).join('. ').replace(/["]/g, '´'))
    }
